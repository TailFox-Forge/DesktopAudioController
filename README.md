...
```text
기준 버전: v0.15.0
개발 단계: Phase 15 출력 라우팅 안정화 및 소스 정리
Public 전환: 완료
...
최근 확인 사항:
- 장치 열거는 워커 프로세스로 격리하고, 시작 직후 캐시 스냅샷으로 UI를 먼저 표시
- 프로그램별 볼륨 패널을 연 경우에만 세션을 주기 갱신해 장치 전체 재열거를 피함
- Windows 앱별 출력 정책 API로 프로그램 세션의 출력 장치 변경 지원
- Edge와 Strinova 실사용 로그에서 VoiceMeeter Input / Aux Input 간 실시간 출력 변경 확인
- 앱 내 업데이트 버튼으로 zip 다운로드 / sha256 검증 / 덮어쓰기 적용 자동화
- 디버그 로그는 기본 OFF이며, ON으로 저장하면 시작 단계부터 기록하기 위해 앱을 자동 재시작
...
- 프로그램 세션의 `출력 변경` 메뉴로 해당 앱의 출력 장치를 다른 렌더 장치로 전환할 수 있습니다.
...
- 새 버전이 있으면 상단에 최신 버전 안내와 자동 업데이트 버튼을 표시합니다.
- 업데이트 버튼은 zip 다운로드, sha256 검증, 별도 updater 프로세스 덮어쓰기를 순서대로 수행합니다.
```
  ...
  src/
    DesktopAudioController/
      ...
      Services/
        ...
        ApplicationAudioOutputPolicy.cs                        -> Windows 앱별 출력 장치 정책 변경 내부 API 래퍼
        AudioPolicyEndpointId.cs                               -> 정책 API용 packed render endpoint ID 변환
        AudioSessionOutputChangeCommand.cs                     -> 출력 변경 워커 실행 인수 규약
        AudioSessionOutputChangeWorker.cs                      -> 별도 프로세스에서 앱별 출력 변경 실행
        AutomaticUpdateService.cs                              -> zip 다운로드, sha256 검증, updater 실행
        NativeAudioSessionService.cs                           -> 장치별 세션 열거, 세션 볼륨/음소거, 출력 변경
        ...
        WorkerBackedAudioSessionService.cs                     -> 출력 변경만 워커 프로세스로 격리하는 세션 서비스
      ViewModels/
...
```bash
bash scripts/publish-win-x64.sh v0.15.0-local
```
...
```text
- 일부 UWP/Store 앱 세션은 @{Package?...} 형태 리소스 URI 대신 실행 파일 아이콘으로 폴백할 수 있습니다.
- Store 앱 고유 타일 아이콘까지 정확히 표시하는 Shell API 처리는 별도 개선 범위입니다.
- 앱별 출력 변경은 Windows 내부 정책 API 기반입니다. Edge와 Strinova에서는 실시간 변경을 확인했지만, 일부 앱은 재생 재시작이나 앱 재실행이 필요할 수 있습니다.
```

## 업데이트 내역

<details open>
<summary><strong>v0.15.0 / 출력 라우팅 안정화 및 소스 정리</strong></summary>

```text
- 앱별 출력 변경 성공 경로를 processId + PackedRender endpoint ID 기준으로 정리
- 출력 변경 내부 API 호출부에 Windows 내부 정책 API 사용 배경 주석 추가
- v0.14.x 디버깅 중 남은 미검증 appIdentifier 재시도 경로 제거
- UI 개발 초기에 쓰던 미사용 PlaceholderAudioDeviceCatalogService 제거
- README / 회귀 체크리스트 / v1 출고 기준을 현재 동작 기준으로 최신화
```

</details>

<details>
<summary><strong>v0.14.9 / 앱별 출력 변경 성공 확인</strong></summary>

```text
- 정책 API 대상 장치 ID를 packed render endpoint ID로 변환
- Edge와 Strinova에서 VoiceMeeter Input / Aux Input 간 출력 변경 확인
- 출력 변경 성공 로그에 endpointIdKind=PackedRender 기록
```

</details>

<details>
<summary><strong>v0.13.19 / 디버그 로그 재시작 안내</strong></summary>

```text
- 디버그 로그를 OFF에서 ON으로 바꾸면 저장 전 재시작 안내 문구 표시
- v0.13.18 자동 업데이트 적용 및 신규 로그 동작 확인
- 업데이트 UX 확인을 위한 후속 릴리즈
```

</details>

<details>
<summary><strong>v0.13.18 / 업데이트 UX 및 앱 아이콘 폴백 개선</strong></summary>

```text
- 자동 업데이트 진행 상태를 다운로드, 체크섬, 검증, 적용 준비 단계로 구분해 표시
- @{Package?...} 형태 세션 아이콘 경로가 들어오면 실행 파일 아이콘으로 폴백
- 디버그 로그 OFF 상태에서 DEBUG 로그가 빠지는 실사용 로그 확인
```

</details>

<details>
<summary><strong>v0.13.17 / 디버그 로그 활성화 시 자동 재시작</strong></summary>

```text
- 설정창에서 디버그 로그를 OFF에서 ON으로 바꾸고 저장하면 앱을 자동 재시작
- 재시작 시 --debug-log 인자를 붙여 settings.json 로드 전 단계부터 DEBUG 로그 기록
- 업데이트 메시지 변경 확인을 위한 릴리즈 버전 갱신
```

</details>

<details>
<summary><strong>v0.13.16 / 디버그 로그 토글 및 업데이트 UI 정리</strong></summary>

```text
- 설정창에 "디버그 로그 기록" 옵션 추가
- 디버그 로그 기본값을 OFF로 변경해 세션 polling 같은 반복 DEBUG 로그를 기본 로그에서 제외
- 디버그 로그 ON 상태에서는 worker probe 프로세스에도 debug 로그 설정 전달
- 상단 버튼 영역이 앱 제목을 침범하지 않도록 헤더 레이아웃 정리
- 업데이트 진행 문구가 어색하게 끊기지 않도록 문구와 줄바꿈 폭 조정
```
