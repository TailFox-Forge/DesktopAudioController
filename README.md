...
1. [릴리즈 페이지](https://github.com/TailFox-Forge/DesktopAudioController/releases)에서 `DesktopAudioController-v0.17.0-win-x64.zip`을 다운로드합니다.
2. 원하는 위치에 압축을 풉니다.
...
```bash
sha256sum -c DesktopAudioController-v0.17.0-win-x64.zip.sha256
```
...
설정창의 `진단 패키지 내보내기`를 사용하면 공개 이슈에 첨부하기 쉬운 zip을 만들 수 있습니다.
...
진단 패키지는 경로, 장치 ID, 세션 ID, 프로필 ID, MatchKey 같은 식별자를 마스킹합니다. 공개 이슈에 올리기 전에는 그래도 zip 내용을 한 번 열어보고 민감한 정보가 남아 있지 않은지 확인하는 것을 권장합니다.
